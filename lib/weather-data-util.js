    ...
    _getConditions: function _getConditions(payload, callback) {
        ...
        request.get(requestURI, function(error, response, body) {
            if (error) {
                ...
                const errorPayload = {
                    speech: error + ' : Error when calling weather api. The developer are looking into this',
                    cardText: error + ' : Error when calling weather api. The developer are looking into this'
                };

                return callback(errorPayload, null);
            }
            const data = JSON.parse(body);
            if (data.response.error) {
                const errorPayload = {
                    speech: data.response.error.description,
                    cardText: data.response.error.description
                };
